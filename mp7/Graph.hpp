...
void Graph<V,E>::removeVertex(const std::string & key) {
  ...
  for(int i = 0; i < vertices.size(); i++){
    std::list<edgeListIter> adj = adjList.at(vertices[i]);
    typename std::list<E_byRef>::iterator it_adj = adj.begin();
    for(it_adj = adj.begin(); it_adj != adj.end(); it_adj++){
      if((*it_adj).source_ == vertices[i] || (*it_adj).dest_ == vertices[i]){
        adj.remove(*it_adj);
      }
    }
  }
...
void Graph<V,E>::removeEdge(const std::string key1, const std::string key2) {
  ...
  for(edgeListIter it = edgeList.begin();it!=edgeList.end();it++){ //go through list of edges
    if ((it->source().key()==key1 && it->dest().key()==key2) || (it->source().key()==key2 && it->dest().key==key1)){
...
void Graph<V,E>::removeEdge(const edgeListIter & it) {
  ...
  V source=it->source(),dest=it->dest();
...
bool Graph<V,E>::isAdjacent(const std::string key1, const std::string key2) const {
  ...
  for(typename std::list<edgeListIter>::iterator it = list.begin();it!=list.end();it++){ //go through all key1's adjacent edges
    if((*(*it)).get().dest()==key2) return true; //if one of those edges destination is key2...they're adjacent!
  }
