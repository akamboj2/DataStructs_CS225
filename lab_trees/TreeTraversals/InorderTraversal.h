...
class InorderTraversal : public TreeTraversal<T> {
  public:
    ...
    {
      //stack.push(root);
			for(typename BinaryTree<T>::Node* i = root;i!=NULL;i=i->left) stack.push(i);
    }
    ...
    void add(typename BinaryTree<T>::Node *& treeNode) {
      // your code here
      if (treeNode->right != NULL) {//first push the thing on the right if there is something
          stack.push(treeNode->right);
          for(typename BinaryTree<T>::Node *& at =treeNode->right->left; at!=NULL;at=at->left) {
            //then push things on left if equal to null
              stack.push(at);
          }
      }
    }
